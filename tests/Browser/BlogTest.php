...
class BlogTest extends DuskTestCase
{
    ...
    public function testCreateBlog(): void
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser->loginAs($this->adminUser)
                ...
                ->attach('.filepond--browser', __DIR__ . '/photos/blog-2.jpg')
                ->pause(4000)
    ...
    public function testEditBlog(): void
    {
        $this->browse(function (Browser $browser) {
            ...
            $blogDescription = $faker->text(20);
            $browser->loginAs($this->adminUser)
                ->visit(BlogResource::getUrl('edit', ['record' => Blog::latest()->first()->id]))
                ->typeSlowly('#data\.title', $blogTitle, 50)
                ->click('#data\.blog_category_id')
                ->pause(2000)
                ->select('#data\.blog_category_id', $blogCategoryId)
                ->click('.filepond--file-action-button.filepond--action-remove-item')
                ->pause(2000)
                ->attach('.filepond--browser', __DIR__ . '/photos/blog-3.png')
                ->pause(4000)
                ->typeSlowly('#data\.content', $blogDescription, 50)
                // ->uncheck('#data\.is_published')
                ->pause(2000)
                ->press('Save changes')
                ->pause(2000)
                ->visit(BlogResource::getUrl('index'));
